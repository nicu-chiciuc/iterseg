...
var Arrow, Canvas, Circle, Drawing, GLB, Grid, LineLayer, Linker, run,
...
Circle = (function() {
  function Circle(layer, x, y, innerCol, outterCol) {
    var mouseDown, mouseEnd, mouseMove, setupGroup;
    mouseDown = (function(_this) {
      return function(x, y, evt) {
        ...
        if (evt.ctrlKey === false) {
          if (nowType === 'left') {
            _this.dragType = 'left';
          } else if (nowType === 'right' && layer.canCreate) {
            _this.dragType = 'right';
            box = _this.group.getBBox();
            layer._crLine.now = 'dragging';
            layer._crLine.stX = box.cx;
            layer._crLine.stY = box.cy;
            layer._crLine.start = _this;
          } else if (nowType === 'middle') {
            if (layer.canCreate) {
              layer.removeCircle(_this);
            }
          }
        } else {
          if (nowType === 'left' && _this.canBeRandom) {
            _this.dragType = 'random';
            _this.varCirc.attr({
              r: Utils.dist2points(_this.getCx(), _this.getCy(), x, y)
            });
          }
          if (nowType === 'middle') {
            _this.varCirc.attr({
    ...
    mouseMove = (function(_this) {
      return function(dx, dy, nx, ny, evt) {
        var a, vec;
        if (_this.dragType === 'left') {
          vec = GLB.grid.gridSnap(nx, ny);
          _this.moveTo(vec.x, vec.y);
          layer.updateArrows();
        }
        if (_this.dragType === 'right' && layer.canCreate) {
          a = Utils.getArrow(layer._crLine.stX, layer._crLine.stY, nx, ny, 15);
          Utils.setpoly(layer._crLine.arrow, a);
        } else if (_this.dragType === 'random') {
          _this.varCirc.attr({
            r: Utils.dist2points(_this.getCx(), _this.getCy(), nx, ny)
          });
        }
        GLB.drawDynamic();
      };
    ...
    mouseEnd = (function(_this) {
      return function(evt) {
        layer.lastCircle = _this;
        if (_this.dragType === 'left') {

        } else if (_this.dragType === 'right' && layer.canCreate) {
          layer._crLine.now = '';
          layer._crLine.arrow.attr({
            points: ''
          });
        }
        _this.dragType = 'none';
    ...
    setupGroup = (function(_this) {
      return function() {
        var circle;
        circle = _this;
        return layer.snap.g(_this.outterCirc, _this.innerCirc).drag(mouseMove, mouseDown, mouseEnd).mouseup(function() {
          var indEnd, indStart;
          if (layer.canCreate) {
...
LineLayer = (function() {
  function LineLayer(col, base) {
    var createInitialCircles, setClickableBackground;
    setClickableBackground = (function(_this) {
      return function() {
        var backEnd, backMove, backStart, layer, stC, stP;
        layer = _this;
        stP = new Vector();
        stC = [];
        backMove = function(dx, dy, nx, ny, evt) {
          var c, i, _i, _ref;
          for (i = _i = 0, _ref = layer.circles.length; _i < _ref; i = _i += 1) {
            c = layer.circles[i];
            console.log(c.getCx());
            c.moveTo(stC[i].x + nx - stP.x, stC[i].y + ny - stP.y);
          }
          return layer.updateArrows();
        };
        backStart = function(x, y, evt) {
          ...
          _results = [];
          for (i = _i = 0, _ref = layer.circles.length; _i < _ref; i = _i += 1) {
            _results.push(stC[i] = new Vector(layer.circles[i].getCx(), layer.circles[i].getCy()));
          }
          return _results;
        };
        backEnd = function(evt) {};
        return _this.snap.rect(0, 0, GLB.width, GLB.height).attr({
          'fill-opacity': 0
